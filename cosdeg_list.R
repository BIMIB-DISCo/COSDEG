...
meta_vars <- function(metadata_df, var_comparison_sel, var_strat_sel) {
  ...
  names(var_comparison_sel_values) <- var_comparison_sel 
  
  (all_var_strat <- lapply(as.list(metadata_df),function(X) {unique(X)}))
  ##
  ...
  #var_strat_sel <- setdiff(var_strat_sel,var_strat_remove)  
  ##if by chance the var_comparison_sel is changed so to include a stratified variable then it is removed from var_strat_sel
  var_strat_sel <- mapply(function(X,Y) { X[which(!X==Y)] }, X=var_strat_sel, Y=var_comparison_sel_values)
  
  
  #df1 <- df0[rowSums(sapply(df0, `%in%`, var_strat_sel)) >= length(var_strat_sel), , drop=FALSE] 
  #df1 contains only rows with all the var_strat_sel
  (df1 <- df0[rowSums(mapply(function(X,Y) { X==Y }, X=a, Y=var_strat_sel)) >= length(var_strat_sel), , drop=FALSE])
  ...
  var_non_strat_null <- colnames(df1[,sapply(df1, function(x) length(unique(x)))==1, drop=FALSE])
  var_non_strat_null
  ...
  #df2 contains rows with non stratified variables
  
  var_strat <- lapply(as.list(df2), function(x) unique(x))
